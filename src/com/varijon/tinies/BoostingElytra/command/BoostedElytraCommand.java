...
public class BoostedElytraCommand implements CommandExecutor
{
	BoostingElytra plugin;
	public BoostedElytraCommand(BoostingElytra plugin) 
	{
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command var2, String label, String[] args) 
	{
		...
		if(args.length > 0)
		{
			...
			if(args[1].equals("armored"))
			{
				ItemStack armoredElytra = Bukkit.getRecipe(new NamespacedKey(plugin, "armored_elytra")).getResult();
				if(!targetPlayer.getInventory().addItem(armoredElytra).isEmpty())
				{
					targetPlayer.getWorld().dropItem(targetPlayer.getLocation(), armoredElytra);
				}
				return true;
			}
			...
			for(int x=1;x<args.length;x++)
			{
				...
				dataContainer.set(NBTKeys.isBoostingElytra, PersistentDataType.INTEGER, 1);		
			...
			elytra = Util.getAndCompleteBoostingElytra(elytra, plugin);
			if(!targetPlayer.getInventory().addItem(elytra).isEmpty())
			{
				targetPlayer.getWorld().dropItem(targetPlayer.getLocation(), elytra);
			}
			return true;
		}
	...
	{
		sender.sendMessage(ChatColor.RED + "Usage: /elytraboosted <playername> <key:value> [key:value] etc.");		
		sender.sendMessage(ChatColor.RED + "To get the default armored elytra, use: /elytraboosted <playername> armored");	
	}
